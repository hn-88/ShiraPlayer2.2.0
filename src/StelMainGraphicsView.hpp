...
#include "recordutils/glthread.h"
#include "recordutils/QVideoEncoder.h"
//
...
public:
    ...
    //ASAF
    QImage getScreenasImage();
    QImage imagescreen;

    //ASAF
    QGLPixelBuffer* m_pStellaBuffer;  //!< Off-screen buffer .
    unsigned int m_stellaTextureID;
    QGLPixelBuffer* getOpenGLBuffer()
    {
        m_stellaTextureID = m_pStellaBuffer->generateDynamicTexture();
        m_pStellaBuffer->bindToDynamicTexture(m_stellaTextureID);
        return m_pStellaBuffer;
    }
    ...
    ewarpMode pwarpMode;
    void setwarpMode(const QString& smode);
    ewarpMode getwarpMode(){return pwarpMode;}
    bool m_bDesignWarp;

    //Warp* m_pDistWarp;              //!< Distortion map warping.
    //Warp* m_pBlendWarp;             //!< Blend map warping.

    //Warp* getCurrentWarp() const;

    void saveWarpSettings(char* filename);
    void loadWarpSettings(QString filename);

    unsigned int getBlendingAreaTexture();
    unsigned int m_blendAreaTexIndex;

    QList<Channel*> m_pChannels;    //!< Channel objects.
    Channel* m_pSelectedChannel;    //!< Selected channel object.
    void removeChannel(int index);

    bool isClientConf;

    void startLoopTimer(bool status);
public slots:
    ...
    void saveScreenShot(const QString& filePrefix="shiraplayer-", const QString& saveDir="");
    ...
    //ASAF
    void setClientConf(bool b){isClientConf = b;}
    //video record i�in
    void update_record();
...
private slots:
    ...
    //Record i�in //ASAF
    void trigencodeImage(QImage img);
...
public:
    ...
    QVideoEncoder v_encode;
    bool b_save;
    QTimer *timer_record ;

    void startRecordFile(QString filename, int FPS = 15);
    void stopRecordFile();
    bool pauseRecord(bool state);
};
