...
SFont& StelFontMgr::getStandardFont(const QString& langageName, double size)
{
	...
	else
	{
		try
		{
                    SFont* font = new SFont((double)lf.size/10, StelFileMgr::findFile("data/"+lf.fileName));
			loadedFonts[lf]=font;
			return *font;
		}
		catch (std::runtime_error& e)
...
{	
	...
	while (!file.atEnd()) {
		...
		if (!commentRx.exactMatch(line))
		{
			...
			try
			{
                            StelFileMgr::findFile("data/" + readFont.fontFileName);
                            StelFileMgr::findFile("data/" + readFont.fixedFontFileName);
			}
			catch(std::runtime_error& e)
			{
				continue;
