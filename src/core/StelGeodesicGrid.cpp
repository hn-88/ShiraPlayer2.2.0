                               ...
                               int maxSearchLevel) const
{
	...
	for (int h=0;h<convex.size();h++)
	{
		...
		for (int i=0;i<12;i++)
		{
            corner_inside[i][h] = half_space.contains(icosahedron_corners[i].toVec3d());
		}
	}
	for (int i=0;i<20;i++)
	{
		...
		**border_list = index;
		if (lev < maxSearchLevel)
		{
			...
			for (int h=0;h<halfs_used_count;h++)
			{
                ...
                edge0_inside[i] = half_space.contains(t.e0.toVec3d());
                edge1_inside[i] = half_space.contains(t.e1.toVec3d());
                edge2_inside[i] = half_space.contains(t.e2.toVec3d());
			}
