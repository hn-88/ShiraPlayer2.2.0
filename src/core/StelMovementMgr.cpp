...
void StelMovementMgr::init()
{
    ...
    //Bu parametre her zaman true yapýldý
    ...
    //InitFov ve InitViewPos sabitlendi
    //initFov = conf->value("navigation/init_fov",60.).toDouble();
    initFov = 180.0;
    currentFov = initFov;
    if (conf->value("navigation/flag_start_initview",false).toBool())
        initViewPos = StelUtils::strToVec3f(conf->value("navigation/init_view_pos").toString()).toVec3d();
    else
        initViewPos = StelUtils::strToVec3f("0.01,0,1").toVec3d();

    //Tilt ekleniyor
    float atilt = conf->value("navigation/tilt_angle",0).toDouble()*M_PI/180.0;
    Vec3d aim;
...
{
    upVectorMountFrame = j2000ToMountFrame(up.toVec3d());
}

void StelMovementMgr::setFrontViewDirection(QString direction, double trackTime)
{
    if (frontDirection == direction) return;

    //Zaman ilerleyiþi real time'a alýndý
    double s = StelApp::getInstance().m_timedirection*JD_SECOND;
    ...
    float atilt = core->tilt_angle*M_PI/180.0;
    ...
    dAlt = dAlt - atilt;
    StelUtils::spheToRect(dAzi,dAlt,aim);
    setMovementSpeedFactor(trackTime/ 100.0);//0.05);
    moveToJ2000(core->getNavigator()->altAzToJ2000(aim));
...
void StelMovementMgr::handleMouseWheel(QWheelEvent* event)
{
    ...
    fov_changed = getAimFov()-mouseZoomSpeed*numSteps*getAimFov()/60.;
...
void StelMovementMgr::autoZoomIn(float moveDuration, bool allowManualZoom)
{
    if (StelMainWindow::getInstance().getIsServer())
        StelApp::getInstance().addNetworkCommand("StelMovementMgr.autoZoomIn("+QString("%0").arg(moveDuration)+");");
    if (!objectMgr->getWasSelected())
        return;
...
{
    if (StelMainWindow::getInstance().getIsServer())
        StelApp::getInstance().addNetworkCommand("StelMovementMgr.autoZoomOut("+QString("%0,%1").arg(moveDuration).arg(full)+");");
    moveDuration /= movementsSpeedFactor;
    ...
    {
        //Vec3d init = getInitViewingDirection();
        //QMessageBox::information(0,"",QString("%0-%1-%2").arg(init.v[0]).arg(init.v[1]).arg(init.v[2]),0,0);
        if (frontDirection!= "")
        {
            //if (!full)
            setFrontViewDirection(frontDirection,50);
        }
        else
...
void StelMovementMgr::setFlagTracking(bool b)
{
    ...
    else
    {
        ...
        flagTracking=true;
        //StelApp::getInstance().addNetworkCommand(QString("StelMovementMgr.setFlagTracking(true);"));
    }
}
...
void StelMovementMgr::moveToObject(const StelObjectP& target, float moveDuration, int zooming)
{
    ...
    /* float a = core->tilt_angle*M_PI/180.0;
    Vec3d aim = target->getAltAzPos(core->getNavigator());
    aim.set(aim.v[0], aim.v[1],aim.v[2]+a);
    aim = core->getNavigator()->altAzToJ2000(aim);
...
void StelMovementMgr::moveToAltAz(const Vec3d &target)
{
    double alt, azi;
    StelUtils::rectToSphe(&azi, &alt, target);
    alt = (alt)*180/M_PI; // convert to degrees from radians
    azi = std::fmod((((azi)*180/M_PI)*-1)+180., 360.);
    if (alt >89.9)alt = 89.9;
    Vec3d aim;
    double dAlt = StelUtils::getDecAngle(QString("%0").arg(alt));
    double dAzi = M_PI - StelUtils::getDecAngle(QString("%0").arg(azi));
    StelUtils::spheToRect(dAzi,dAlt,aim);
    core->getMovementMgr()->moveToJ2000(core->getNavigator()->altAzToJ2000(aim));
...
void StelMovementMgr::panView(double deltaAz, double deltaAlt)
{
    ...
    //ASAF
    rec_deltaAz = rec_deltaAz + deltaAz;
    rec_deltaAlt= rec_deltaAlt + deltaAlt;
}
...
void StelMovementMgr::zoomTo(double aim_fov, float moveDuration)
{
    ...
    //StelApp::getInstance().addNetworkCommand( QString("StelMovementMgr.setFlagTracking(true);StelMovementMgr.zoomTo(%0,%1);").arg(aim_fov).arg(moveDuration) );
}
