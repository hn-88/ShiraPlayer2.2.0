...
#define JD_DAY    1.
#define ONE_OVER_JD_SECOND 24 * 60 * 60
...
public:
    ...
    void moveObserverTo(const StelLocation& target,
                        double duration=1., double durationIfPlanetChange=1., bool isFlyby = false);
    ...
    //ASAF
    const Mat4d& getScreenViewMat() const {return matAltAzToScreen;}
    ...
    //ASAF
    static const Mat4d matAltAzToScreen;
    bool firstPass;
    void setStelObserver(StelObserver* _position) {position = _position; }
public slots:
    ...
    {
        setTimeRate(0);
        // StelApp::getInstance().addNetworkCommand("core.setTimeRate(0);");
    }
    ...
    void setRealTimeSpeed()
    {
        ...
        StelApp::getInstance().addNetworkCommand("core.setTimeRate("+ QString("%0").arg(JD_SECOND) +");");
