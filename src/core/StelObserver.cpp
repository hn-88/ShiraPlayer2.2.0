...
ArtificialPlanet::ArtificialPlanet(const PlanetP& orig) :
        ...
        NULL, NULL, 0, false, true, false,Vec3f(0,0,0)), dest(0),
...
StelObserver::StelObserver(const StelLocation &loc) : currentLocation(loc)
{
    ...
    planet = ssystem->searchByEnglishNameForFlyby(loc.planetName);
	if (planet==NULL)
	{
...
{
	SolarSystem* ssystem = GETSTELMODULE(SolarSystem);
    PlanetP targetPlanet;
    targetPlanet = ssystem->searchByEnglishNameForFlyby(moveTargetLocation.planetName);
    if (moveStartLocation.planetName!=moveTargetLocation.planetName)
	{
        PlanetP startPlanet;
        startPlanet = ssystem->searchByEnglishNameForFlyby(moveStartLocation.planetName);
