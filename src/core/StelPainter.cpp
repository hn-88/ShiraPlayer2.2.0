...
void StelPainter::sRing(float rMin, float rMax, int slices, int stacks, int orientInside)
{
    ...
    if (isLightOn)
    {
        ...
        lightPos3 = prj->modelViewMatrix.transpose().multiplyWithoutTranslation(lightPos3).toVec3d();
        lightPos3.normalize();
...
void StelPainter::sSphereMap(float radius, int slices, int stacks, float textureFov, int orientInside)
{
    ...
    {
        for (i = 0,cos_sin_rho_p=cos_sin_rho,rho=0.f; i < imax; ++i,cos_sin_rho_p+=32,rho+=drho)
        {
            vertexArr.resize(0);
            texCoordArr.resize(0);
            for (j=0,cos_sin_theta_p=cos_sin_theta;j<=slices;++j,cos_sin_theta_p+=32)
            {
...
void StelPainter::drawTextGravity180(float x, float y, const QString& ws, float xshift, float yshift) const
{
    ...
    //    double scale=f.pixelSize()/16.25; //Istenilen Font Büyüklüðünü hesaplamak
    //
...
void StelPainter::drawText(float x, float y, const QString& str, float angleDeg, float xshift, float yshift, bool noGravity) const
{
    ...
    {
        //QByteArray textTemp(str.toUtf8() );
        //        qDebug()<<textTemp;
        //QByteArray textTemp = QTextCodec::codecForName("EUC-JP")->fromUnicode(str);
        //japaneseCodec->toUnicode(str);
    ...
    else
    {
        ...
        {
            sFont->printChar(str[i]);
            glTranslatef(sFont->getStrLen(str.mid(i,1)) * 1.05f, 0, 0);
        }
        glPopMatrix();
        glPopAttrib();
                                           ...
                                           double maxSqDistortion,int nbI, bool checkDisc1, bool checkDisc2, bool checkDisc3) const
{
    ...
    if (!cDiscontinuity1 && !cDiscontinuity2 && !cDiscontinuity3)
    {
        ...
        outVertices->append(Vec3f(e0[0], e0[1], e0[2]));
        outVertices->append(Vec3f(e1[0], e1[1], e1[2]));
        outVertices->append(Vec3f(e2[0], e2[1], e2[2]));
        if (outTexturePos)
    ...
    if (nbI > 4)
    {
        ...
        outVertices->append(Vec3f(e0[0], e0[1], e0[2]));
        outVertices->append(Vec3f(e1[0], e1[1], e2[2]));
        outVertices->append(Vec3f(e2[0], e2[1], e2[2]));
        if (outTexturePos)
            outTexturePos->append(texturePos,3);
        return;
...
void StelPainter::DrawStrokePath(float x1, float y1, float x2, float y2)
{
...
void StelPainter::DrawStrokePath(QPainterPath _path,QPen _pen,int _size,double _opacity)
{
    Q_ASSERT(qPainter);
    ...
    qPainter->resetTransform();
    qPainter->resetMatrix();
    //qPainter->setRenderHints(QPainter::Antialiasing | QPainter::TextAntialiasing | QPainter::SmoothPixmapTransform | QPainter::NonCosmeticDefaultPen | QPainter::HighQualityAntialiasing);
    //qPainter->setRenderHints(QPainter::HighQualityAntialiasing);
    qPainter->setRenderHints(QPainter::NonCosmeticDefaultPen );

    //qPainter->setRenderHint(QPainter::Antialiasing,true);
    //qPainter->setRenderHint(QPainter::TextAntialiasing,true);
    //qPainter->setRenderHint(QPainter::SmoothPixmapTransform ,true);

    _pen.setColor(_pen.color());
    _pen.setWidth(_size);
    _pen.setCapStyle( Qt::RoundCap);
    _pen.setJoinStyle(Qt::RoundJoin);
    qPainter->setPen(_pen);

    qPainter->setOpacity(_opacity);
    qPainter->drawPath(_path);
...
void StelPainter::sSphere2(float radius, float oneMinusOblateness, int slices, int stacks, bool flipTexture,
                           double elipsisrate)
{
    GLfloat x, y, z;
    GLfloat s=0.f, t=0.f;
    ...
    for (i = 0,cos_sin_rho_p = cos_sin_rho; i < stacks; ++i,cos_sin_rho_p+=2)
    {
        ...
        for (j = 0,cos_sin_theta_p = cos_sin_theta; j<=slices;++j,cos_sin_theta_p+=2)
        {
            ...
            vertexArr << x * radius << y * radius << z * oneMinusOblateness * radius * elipsisrate;
            x = -cos_sin_theta_p[1] * cos_sin_rho_p[3];
            y = cos_sin_theta_p[0] * cos_sin_rho_p[3];
            z = nsign * cos_sin_rho_p[2];
            texCoordArr << s << t - dt;
...
void StelPainter::sCylinder2(float radius, float height, int slices, int orientInside)
{
    if (orientInside)
        glCullFace(GL_FRONT);
    ...
    for (int i = 0; i <= slices; ++i)
    {
        ...
        y = std::cos(da*i);
        texCoordArray.append(Vec2f(s, 0.f));
        vertexArray.append(Vec3d(x*radius, y*radius, - height / 2));
        texCoordArray.append(Vec2f(s, 1.f));
        vertexArray.append(Vec3d(x*radius, y*radius, height / 2));
        s += ds;
    }
