...
void StelSkyDrawer::init()
{
    ...
    texFlyByAtmosphere = StelApp::getInstance().getTextureManager().createTexture("textures/neb.png");
...
void StelSkyDrawer::postDrawSky3dModelAtm(StelPainter *sPainter, const Vec3d &v, float illuminatedArea, float mag, const Vec3f &color)
{
    ...
    float maxHaloRadius = qMax(tStart*4., pixRadius*4.);
    if (rcm[0]>maxHaloRadius)
    {
        //truncated = true;
        rcm[0]=maxHaloRadius+std::sqrt(rcm[0]-maxHaloRadius);
    ...
    //if (!noStarHalo)
    //{
    preDrawPointSource(sPainter);
    drawPointSourceAtm(sPainter, v,rcm,color);
    postDrawPointSourceAtm(sPainter);
    //}
    flagStarTwinkle=save;
}


bool StelSkyDrawer::drawPointSourceAtm(StelPainter *sPainter, const Vec3d &v, const float rcMag[], const Vec3f &color, bool checkInScreen)
{
    Q_ASSERT(sPainter);
...
void StelSkyDrawer::postDrawPointSourceAtm(StelPainter *sPainter)
{
    Q_ASSERT(sPainter);
...
void StelSkyDrawer::setBortleScale(int bIndex)
{
    ...
    StelApp::getInstance().addNetworkCommand("StelSkyDrawer.setBortleScale("+QString("%0").arg(bIndex)+");");
}
