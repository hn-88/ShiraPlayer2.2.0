...
void ScriptConsole::languageChanged()
{
	if (dialog)
    {
        ...
        ui->stelWindowTitle->setText(q_( "Script console"));
        ...
        ui->preprocessSSCButton->setToolTip(q_( "pre-process script using SSC preprocessor"));
        ui->preprocessSSCButton->setText(q_( "SSC"));

        ui->preprocessSTSButton->setToolTip(q_( "pre-process script using STS preprocessor"));
        ui->preprocessSTSButton->setText(q_( "STS"));

        ui->quickrunCombo->setToolTip(q_( "quickly load and run a utility script"));
        ...
        ui->tabs->setStyleSheet(q_( "QTabBar::tab {\n"
                                    "	min-height: 28px;\n"
                                    "}"));
        ui->tabs->setTabText(ui->tabs->indexOf(ui->scriptTab), q_( "Script"));
        ui->tabs->setTabText(ui->tabs->indexOf(ui->outputTab), q_( "Output"));

        ui->rowColumnLabel->setToolTip(q_( "Cursor position"));
        ui->rowColumnLabel->setText(q_( "R:0 C:0"));
        ui->label->setText(q_( "Include dir:"));
        ui->includeBrowseButton->setText(q_( "..."));
...
void ScriptConsole::prepareAsChild()
{
    ui->LocationBar->hide();
}
...
void ScriptConsole::on_visibleChanged(bool b)
{
   StelApp::getInstance().isScriptConsole = b;
}
void ScriptConsole::on_load(bool b)
{
