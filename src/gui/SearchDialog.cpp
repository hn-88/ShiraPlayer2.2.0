...
void SearchDialog::languageChanged()
{
    if (dialog)
    {
        QString text(ui->lineEditSearchSkyObject->text()); // Ã–nce alÄ±nÄ±yor

        //ui->retranslateUi(dialog);
        ui->chMove->setText(q_("Move"));
        ui->raDecLabel->setText(q_("RA/Dec (J2000):"));

        ui->lineEditSearchSkyObject->setText(text);// Sonra yapistiriliyor
    }
}
...
void SearchDialog::prepareAsChild()
{
    ui->TimeBar->hide();
}
...
void SearchDialog::gotoObject()
{
    ...
    if (simbadResults.contains(name))
    {
        ...
        //Simbad aramalarý client tarafýnda gösterilemiyor
        // Burasý düzeltilecek
        //qDebug()<<"simbad bulundu";
        //---
        /*
        const Vec3d& current = pos;
        double alt, azi;
        ...
        Sleep(100);
        StelApp::getInstance().addNetworkCommand("core.moveToRaDecJ2000("+QString("%0").arg(alt)+","+QString("%0").arg(azi)+",2);");
        //--
        */
    }
    else if (objectMgr->findAndSelectI18n(name))
    {
        ...
        if (!newSelected.empty())
        {
            ...
            {
                QString s = newSelected[0]->getEnglishName();
                ...
                if (s=="")
                {
                    s = newSelected[0]->getInfoString(StelApp::getInstance().getCore(),infoTextFilters);
                    s = s.replace("\n","");
                }
                StelApp::getInstance().addNetworkCommand("core.selectObjectByName(\""+s.replace("\n"," ")+"\",true);");
                ...
                if(ui->chMove->isChecked())
                {
                    ...
                    Sleep(100);
                    StelApp::getInstance().addNetworkCommand("core.moveToAltAzi("+QString("%0").arg(alt)+","+QString("%0").arg(azi)+",2);");
                    //--
                }
            }
            else
    ...
    simbadResults.clear();
    StelMainWindow::getInstance().getPreviewWidget()->setFocus();
}
...
QString SearchDialog::getGreekLetterByName(const QString& potentialGreekLetterName)
{
    ...
    // There can be indices (e.g. "Î±1 Cen" instead of "Î± Cen A"), so strip
    // any trailing digit.
