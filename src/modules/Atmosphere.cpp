                              ...
                              StelCore* core, float latitude, float altitude, float temperature, float relativeHumidity)
{
        ...
        for (int x=0; x<=skyResolutionX; ++x)
        {
            for(int y=0; y<=skyResolutionY; ++y)
            {
                ...
                v[0] = viewport_left + ((x == 0) ? 0.f :
                       (x == skyResolutionX) ? (float)prj->getViewportWidth() :
                       (x-0.5*(y&1))*stepX);
    ...
    sky.setParamsv(sunPos, 4.0f);//ASAF Orjinali 1.0 dý.
...
void Atmosphere::draw(StelCore* core)
{
    ...
    //sPainter.setProjector(core->getProjection(core->getNavigator()->getAltAzModelViewMat()));
    //sPainter.setProjector(core->getProjection(core->getNavigator()->getAltAzModelViewMat() * Mat4d::translation(Vec3d(0.,0.,-1.0))));
    ...
    if (useShader)
    {
        ...
        Mat4f m = sPainter.getProjector()->getProjectionMatrix();

        /* FLYBY durumunda gezegen atmosferi oluþturmak için düþünülmüþtü, iptal edildi
        //--asaf
        StelNavigator* nav =  core->getNavigator();
        StelLocation loc = nav->getCurrentLocation();
        SolarSystem* ssystem = GETSTELMODULE(SolarSystem);
        StelProjectorP proj = sPainter.getProjector();

        /*Mat4f m = Mat4f();
        if (loc.planetName.indexOf("FLYBY")>-1)
        {
            PlanetP flyPlanet = ssystem->searchByEnglishNameForFlyby(loc.planetName);
            PlanetP byPlanet = flyPlanet->getFlybyPlanet();
            double rateAltitude = byPlanet->getRadius() / flyPlanet->getRadius() ;
            Vec3d flypos = byPlanet->getScreenPos();
            m = proj->getProjectionMatrix();
            //m = m * Mat4f::translation(flypos[0]);
            m = m * Mat4f::scaling(Vec3f(rateAltitude,rateAltitude,rateAltitude));
            m = m * Mat4f::translation(Vec3f(flypos[0] - proj->getViewportWidth()*rateAltitude / 2.f,
                                             flypos[1],0));
           //                                  flypos[1] - proj->getViewportHeight()*rateAltitude / 2.,0));

            //m = m *Mat4f::scaling(Vec3f(.5f,.5f,.5f));
                    //Mat4f::translation(Vec3f((flypos[0]-flypos.length()/2.),
                    //                         (flypos[1]),0))*
            //m = m* Mat4f::scaling(Vec3f(rateAltitude,rateAltitude,rateAltitude));
            //m= m * Mat4f::translation(Vec3f(flypos[0]/2.,flypos[1]/2.,0));
            //m= m * Mat4f::translation(Vec3f(flypos));
                    //Mat4f::scaling(Vec3f(1.f,1.f,1.f))*
                    //Mat4f::translation(Vec3f(flypos[0]/2.,flypos[1]/2.,0));

            qDebug()<<flypos.toString();
           // qDebug()<<proj->getViewport()[3] <<"-"<<proj->getViewport()[4];
            qDebug()<<proj->getViewportWidth()<<"-"<<proj->getViewportHeight();
            qDebug()<<"rateAlti"<<rateAltitude;
            qDebug()<<"Fly radius"<< flyPlanet->getRadius()*AU;
            qDebug()<<"By radius"<< byPlanet->getRadius()*AU;
            //qDebug()<<"Fly sphere"<<flyPlanet->getSphereScale();
            //qDebug()<<"By sphere"<<byPlanet->getSphereScale();

        }
        else
        {
            m = sPainter.getProjector()->getProjectionMatrix();
        }

        /*qDebug()<<"flypos lonlat:"<<flypos.toStringLonLat();
        qDebug()<<"fly normal"<<flypos.toString();
        qDebug()<<"pixelperrad"<<sPainter.getProjector()->getPixelPerRadAtCenter();*/
        //--
