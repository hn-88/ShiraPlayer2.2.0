...
void viewportEdgeIntersectCallback(const Vec3d& screenPos, const Vec3d& direction, void* userData)
{
	...
	if (angleDeg>90. || angleDeg<-90.)
	{
        ...
        //ASAF
        if(StelMainGraphicsView::getInstance().isClientConf)
...
{
    if (StelApp::getInstance().getCore()->skyline_width <= 5)
        glLineWidth(StelApp::getInstance().getCore()->skyline_width);
    else
        glLineWidth(1);
...
void GridLinesMgr::setFlagAzimuthalGrid(bool b)
{
    aziGrid->setFlagshow(b);
    StelApp::getInstance().addNetworkCommand("GridLinesMgr.setFlagAzimuthalGrid("+QString("%0").arg(b)+");");
}
...
void GridLinesMgr::setFlagEquatorGrid(bool b)
{
    equGrid->setFlagshow(b);
    StelApp::getInstance().addNetworkCommand("GridLinesMgr.setFlagEquatorGrid("+QString("%0").arg(b)+");");
}
...
void GridLinesMgr::setFlagEquatorJ2000Grid(bool b)
{
    equJ2000Grid->setFlagshow(b);
    StelApp::getInstance().addNetworkCommand("GridLinesMgr.setFlagEquatorJ2000Grid("+QString("%0").arg(b)+");");
}
...
void GridLinesMgr::setFlagEquatorLine(bool b)
{
    equatorLine->setFlagshow(b);
    StelApp::getInstance().addNetworkCommand("GridLinesMgr.setFlagEquatorLine("+QString("%0").arg(b)+");");
}
...
void GridLinesMgr::setFlagEclipticLine(bool b)
{
    eclipticLine->setFlagshow(b);
    StelApp::getInstance().addNetworkCommand("GridLinesMgr.setFlagEclipticLine("+QString("%0").arg(b)+");");
}
...
void GridLinesMgr::setFlagMeridianLine(bool b)
{
    meridianLine->setFlagshow(b);
    StelApp::getInstance().addNetworkCommand("GridLinesMgr.setFlagMeridianLine("+QString("%0").arg(b)+");");
}
