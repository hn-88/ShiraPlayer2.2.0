...
Vec3f Planet::axisColor = Vec3f(0,0,1);
Vec3f Planet::equatorColor = Vec3f(0,0,1);
StelTextureSP Planet::hintCircleTex;
StelTextureSP Planet::texEarthShadow;
StelTextureSP Planet::texAtmosphere;

Planet::Planet(QSharedPointer<Planet> p)
    : englishName(p.data()->englishName),
      flagLighting(p.data()->flagLighting),
      radius(p.data()->radius), oneMinusOblateness(p.data()->oneMinusOblateness),
      color(p.data()->color), albedo(p.data()->albedo), axisRotation(0.), rings(NULL),
      sphereScale(1.f),
      lastJD(J2000),
      coordFunc(p.data()->coordFunc),
      userDataPtr(p.data()->userDataPtr),
      osculatingFunc(p.data()->osculatingFunc),
      parent(NULL),
      hidden(p.data()->hidden),
      atmosphere(p.data()->atmosphere),
      newOrbitColor(p.data()->newOrbitColor)
{
    //coordFunc = &get_sun_helio_coordsv;
    texMapName = p.data()->texMapName;
    lastOrbitJD =0;
    deltaJD = JD_SECOND;
    orbitCached = 0;
    closeOrbit =  p.data()->closeOrbit;
    ...
    //normtexMap = StelApp::getInstance().getTextureManager().createTextureThread("textures/Norm_"+texMapName, StelTexture::StelTextureParams(true, GL_LINEAR, GL_REPEAT));
    ...
    flybyatmFader.setDuration(2000);
    flybyatmFader = true;

    animSphereFader.setDuration(3000);
    animSphereScale = 1.0;
}
      ...
      newOrbitColor(orbitcolor)
{
    ...
    //newOrbitColor = color;
    //qDebug()<<englishName<< orbitColor.toString();
...
{
    //ORbitler ilk anda bozuk çiziliyordu o yüzden bu þart kaldýrýldý
...
{
    // Flyby özelliði gizlenmiþ gezegende olsa radius, radiusFader ile yeniden hesaplanýyor.
    if (radiusFader.getInterstate()>0)
    {
        double farkRadius = radiusforFader - oldradius;
        radius = oldradius + farkRadius* radiusFader.getInterstate();
        if (radiusFader.getInterstate() == 1)
        {
            radiusFader.reset();
        }
    }
    ...
    if (this->englishName != "Moon")
        if ((animSphereFader.getInterstate() < 1) ||
                (animSphereFader.getInterstate() > 0) )
        {
            sphereScale = 1 +
                    animSphereFader.getInterstate()
                    *getAnimSphereScale();
            if (getRings())
                getRings()->setRingScale(sphereScale);
        }
    ...
    float screenSz = getAngularSize(core)*M_PI/180.*prj->getPixelPerRadAtCenter();//* getSphereScale();
    float viewport_left = prj->getViewportPosX();
    float viewport_bottom = prj->getViewportPosY();

    //#ifndef SHIRAPLAYER_PRE
    //    // ASAF Burada iptal edilenler ile 180 derecelik FOV ile tüm ekranlara ayný durumlar yansýtýlýyor
    ...
    //    {
    //#endif
    if (prj->project(Vec3d(0,0,0), screenPos) &&
            screenPos[1]>viewport_bottom - screenSz && screenPos[1]<viewport_bottom + prj->getViewportHeight()+screenSz &&
            screenPos[0]>viewport_left - screenSz && screenPos[0]<viewport_left + prj->getViewportWidth() + screenSz)
    {
        ...
        //Sadece 9 gezegenin orbiti çizidiriliyor
        SolarSystem* ssm = GETSTELMODULE(SolarSystem);
        if ( this == ssm->getMerkur() ||
             this == ssm->getVenus() ||
             this == ssm->getEarth() ||
             this == ssm->getMars() ||
             this == ssm->getJupiter() ||
...
void Planet::draw3dModel(StelCore* core, const Mat4d& mat, float screenSz)
{
    ...
    if (screenSz>1.)
    {
        ...
        if (rings)
        {
            ...
            // sPainter->sCylinder(screenSz,screenSz,64,1);
        ...
        else
        {
            SolarSystem* ssm = GETSTELMODULE(SolarSystem);
            if (this==ssm->getMoon() &&
                    (nav->getCurrentLocation().planetName=="Earth" ||
                     nav->getCurrentLocation().planetName=="FLY-Earth") &&
                    ssm->nearLunarEclipse())
            {
            ...
            {
                //axis lerin doðru görünmesi için gl parametreleri eklendi
                glDepthMask(GL_TRUE);
                glClear(GL_DEPTH_BUFFER_BIT);
                glEnable(GL_DEPTH_TEST);
                drawSphere(sPainter, screenSz);
                ...
                //Dünya ve Mars için Atmosfer çiziliyor- FLYBY durumunda
                if ( ( this == ssm->getEarth() || this == ssm->getMars()) &&
                     ( (nav->getCurrentLocation().planetName=="FLYBY-Earth")
                       ||
                       (nav->getCurrentLocation().planetName=="FLYBY-Mars")))
                {
                    sPainter->getLight().disable();
                    drawAtmosphere(sPainter,screenSz);
                }
    ...
    core->getSkyDrawer()->postDrawSky3dModel(&sPainter,
                                             getJ2000EquatorialPos(nav),
                                             surfArcMin2,
                                             getVMagnitude(core->getNavigator()), color);


}
void Planet::drawAtmosphere(StelPainter* painter, float screenSz)
{
    SolarSystem* ssm = GETSTELMODULE(SolarSystem);
    ...
    int nb_facet = (int)(screenSz * 100/50);
    if (nb_facet<10) nb_facet = 10;
    if (nb_facet>40) nb_facet = 100;
    //qDebug()<< flybyatmFader.getInterstate();
    painter->setShadeModel(StelPainter::ShadeModelSmooth);
    if (this == ssm->getEarth())
        painter->setColor(0.5f,0.5f,0.7f,flybyatmFader.getInterstate() *0.2f);//dünya
    else
        painter->setColor(0.7f,0.5f,0.5f,flybyatmFader.getInterstate()*0.2f);//mars

    painter->sSphere(radius*sphereScale*1.015, oneMinusOblateness, nb_facet, nb_facet);
...
void Planet::drawSphere(StelPainter* painter, float screenSz)
{
    ...
    {
        nb_facet = (int)(screenSz * 100/50);	// 40 facets for 1024 pixels diameter on screen
        if (nb_facet<10) nb_facet = 10;
        if (nb_facet>40) nb_facet = 100;
    }
    painter->setShadeModel(StelPainter::ShadeModelSmooth);
    ...
    if ( StelApp::getInstance().getstartedFlyby() )
    {
        ...
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
        if (flagLighting)
            painter->getLight().disable();
        painter->enableTexture2d(false);
        painter->setColor(axisColor[0], axisColor[1], axisColor[2], axisFader.getInterstate());
        painter->sCylinder2(radius*sphereScale/screenSz*1.5,2*radius*sphereScale*1.2, 64, 1);
        //axis
        //equator line
        if ( (englishName != "Saturn") &&
             (englishName != "Moon") )
        {
            painter->setColor(equatorColor[0], equatorColor[1], equatorColor[2], equatorFader.getInterstate());
            //if ( StelApp::getInstance().getstartedFlyby() )
            painter->sRing(radius*sphereScale*1.1,radius*sphereScale*1.12,nb_facet, nb_facet, 0);
            /*else
        {
            glDepthMask(GL_FALSE);
            glDisable(GL_DEPTH_TEST);
            painter->sCylinder2(radius*sphereScale*1.1,radius*sphereScale/screenSz*3,nb_facet,0);
        }*/
...
void Planet::drawOrbit(const StelCore* core)
{
    ...
    sPainter.setColor(newOrbitColor[0], newOrbitColor[1], newOrbitColor[2], orbitFader.getInterstate());
    ...
    QVarLengthArray<float, 2048> vertexArray; // Eskisi 1024 idi
...
void Planet::update(int deltaTime)
{
    ...
    radiusFader.update(deltaTime);
    flybyatmFader.update(deltaTime);
    axisFader.update(deltaTime);
    equatorFader.update(deltaTime);

    animSphereFader.update(deltaTime);
}
...
void Ring::draw(StelPainter* sPainter,const Mat4d& mat,double screenSz)
{
    ...
    const double h = mat.r[ 8]*mat.r[12]
            + mat.r[ 9]*mat.r[13]
            + mat.r[10]*mat.r[14];
    sPainter->sRing(radiusMin*ringScale,radiusMax*ringScale,(h<0.0)?slices:-slices,stacks, 0);
    glDisable(GL_CULL_FACE);
}
