...
StarMgr* StarMgr::singleton = NULL;
...
void StarMgr::recreateTrails()
{
    ...
    {
        const Star1 *const s = hipIndex[i].s;
        if (s)
        {
            if (hipIndex[i].a->level == 0)
            {
                const SpecialZoneArray<Star1> *const a = hipIndex[i].a;
                const SpecialZoneData<Star1> *const z = hipIndex[i].z;
                const Vec3f& trailColor = StelSkyDrawer::indexToColor(s->bV)*0.75f;
                allTrails->addObject((QSharedPointer<StelObject>)s->createStelObject(a,z),&trailColor);
            }
...
void StarMgr::init()
{
    ...
    setFlagSelectedLabel(conf->value("astro/flag_star_selected_name").toBool());
    setLabelsAmount(conf->value("stars/labels_amount",3).toDouble());
    starFont.setPixelSize(conf->value("viewing/star_font_size",13).toInt());
...
void StarMgr::drawPointer(StelPainter& sPainter, const StelNavigator * nav,const float *rcmag_table)
{
    ...
    if (!newSelected.empty())
    {
        ...
        const Vec3d& c(obj->getInfoColor().toVec3d());
        ...
        {
            const Vec3f& colorr = StelSkyDrawer::indexToColor(obj->bV)*0.75f;
            sPainter.setColor(colorr[0], colorr[1], colorr[2]);
            if (obj->getNameI18n() == "")
                sPainter.drawText(screenpos[0], screenpos[1], obj->getEnglishName(), 0, 13, 5, false);
            else
                sPainter.drawText(screenpos[0], screenpos[1], obj->getNameI18n(), 0, 13, 5, false);
        }
        //--
    }
}
...
void StarMgr::setStelStyle(const StelStyle& style)
{
    ...
    QSettings* conf = StelApp::getInstance().getSettings();
    QString section = "color";//style.confSectionName;
...
void StarMgr::draw(StelCore* core)
{
    ...
    if((!StelMainWindow::getInstance().getIsServer()) ||
            (
                ...
                (!StelAppGraphicsWidget::getInstance().isDrawingPreview)
            )
      )
    {
        if (trailFader.getInterstate()>0.0000001)
        {
            ...
            allTrails->setOpacity(trailFader.getInterstate()* starsFader.getInterstate());
    ...
    for (ZoneArrayMap::const_iterator it(zoneArrays.begin()); it!=zoneArrays.end();++it)
    {
        ...
        for (int i=it.value()->mag_steps-1;i>=0;--i)
        {
            ...
            {
                //ASAF bu satýr açýldýðýnda yýldýzlar çizilmiyor
...
exit_loop:
    ...
    if (objectMgr->getFlagSelectedObjectPointer())
        drawPointer(sPainter, nav,rcmag_table);
...
void StarMgr::setFlagTrails(bool b)
{
    trailFader = b;
    if (b)
        allTrails->reset();
...
bool StarMgr::getFlagTrails() const
{
    return (bool)trailFader;
}

void StarMgr::setFlagMagLevel(bool b)
{
    flagMagLevel = b;
}

bool StarMgr::getFlagMagLevel() const
{
    return flagMagLevel;
}
