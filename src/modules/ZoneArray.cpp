                             ...
                             const Vec3f &c2)
{
    ...
    z.center = c0+c1+c2;
    z.center.normalize();
    z.axis0 = north.toVec3f() ^ z.center.toVec3f();
    z.axis0.normalize();
                                  ...
                                  float names_brightness) const
{
    ...
    for (const Star *s=z->getStars();s<end;++s)
    {
        ...
        double vmag = 0.001f*mag_min + s->mag*(0.001f*mag_range)/mag_steps;
        StarMgr* smgr = GETSTELMODULE(StarMgr);
        if (smgr->getFlagMagLevel())
            if (vmag > smgr->getlimitMagValue())
                continue;
        ...
        if ((!newSelected.empty()) && s->hasName())
        {
            const StelObjectP obj = newSelected[0];
            if(obj->getNameI18n() == s->getNameI18n())
            {
                const float offset = *tmpRcmag*0.7f;
                sPainter->drawText(v, obj->getNameI18n(), 0, offset, offset, false);
            }
        }*/
    }
}
