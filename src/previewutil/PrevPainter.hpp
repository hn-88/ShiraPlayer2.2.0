#ifndef PREVPAINTER_HPP
#define PREVPAINTER_HPP
...
class PrevPainterLight
{
public:
        PrevPainterLight(int alight=0) : light(alight), enabled(false) {}
...
//! Because openGL is not thread safe, only one instance of PrevPainter can exist at a time, enforcing thread safety.
//! As a coding rule, no openGL calls should be performed when no instance of PrevPainter exist.
//! Typical usage is to create a local instance of PrevPainter where drawing operations are needed.
...
public:
        ...
        explicit PrevPainter(const StelProjectorP& prj);
        ~PrevPainter();
        ...
        //! Get the light
        PrevPainterLight& getLight() {return light;}

        //! Get the material
        PrevPainterMaterial& getMaterial() {return material;}
...
private:
        ...
        PrevPainterLight light;

        //! The material used by the painter
        PrevPainterMaterial material;
};

#endif // PREVPAINTER_HPP
