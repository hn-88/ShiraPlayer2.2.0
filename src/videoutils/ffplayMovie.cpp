...
static int video_open(VideoState *is){
    int flags = SDL_HWSURFACE | SDL_ASYNCBLIT | SDL_HWACCEL | SDL_DOUBLEBUF |SDL_NOFRAME ;
    int w,h;
...
static int refresh_thread(void *opaque)
{
    ...
    while(!is->abort_request)
    {
        ...
        if(!is->refresh)
        {
            ...
            SDL_PushEvent(&event);
            ////ASAF
            //b_show_refreshed = true;
            //is->refresh = 0;
        }
        Sleep((is->audio_st && is->show_audio ? rdftspeed*1000 : 5000)/1000); //FIXME ideally we should wait the correct time but SDLs event passing is so slow it would be silly
    }
    return 0;
}
...
void video_refresh_timer(VideoState * opaque)
{
    VideoState *is = (VideoState *)opaque;
    VideoPicture *vp;
...
static void FlipImageRGB32(BYTE* pBuf, int width, int height, 
...
static int get_video_frame(VideoState *is, AVFrame *frame, int64_t *pts, AVPacket *pkt)
{
    ...
    if(pkt->data == flush_pkt.data){
        ...
        is->last_dts_for_fault_detection=
                is->last_pts_for_fault_detection= _I64_MIN;
...
static int decode_thread(void *arg)
{
    ...
    for(;;) {
        ...
        pkt_in_play_range = duration == AV_NOPTS_VALUE || (pkt->pts - ic->streams[pkt->stream_index]->start_time) * av_q2d(ic->streams[pkt->stream_index]->time_base) -
...
VideoState* Start_Movie(char* input_filename)
{
    int flags, i;
...
bool get_show_refreshed()
{
    return b_show_refreshed;
}
void set_show_refreshed(bool val)
{
    b_show_refreshed = val;
}
...
int get_movie_size()
{
    return avpicture_get_size(PIX_FMT_RGBA, cur_stream->video_st->codec->width, cur_stream->video_st->codec->height);
}
int getMovieWidth()
{
    if(!cur_stream)
        ...
        if(!cur_stream->video_st->codec)
            return 0;
    else
        if(cur_stream->video_st->codec->width==0)
            return 0;
    else
        return cur_stream->video_st->codec->width;
...
int getMovieHeight()
{
    if(!cur_stream)
        ...
        if(cur_stream->video_st->codec->height==0)
            return 0;
    else
        return cur_stream->video_st->codec->height;

}

AVFrame* get_currFrame()
{
    return pFrameRGB;
}
...
std::string get_properties_str(char* filename)
{
    char buf[50];
    std::string str = "";
    ...
    if(av_open_input_file(&pFormatCtx, (LPSTR)filename, NULL, 0, NULL)!=0)
        return "";
    ...
    //Format Width:Height:FPS:VideoCodecName:duration:Bitrate
    sprintf(buf,"%d", pCodecCtx->width);
    str = buf;
    sprintf(buf,"%d", pCodecCtx->height);
    str = str +":"+ buf;
    sprintf(buf,"%d", video_st->r_frame_rate.num);
    str = str +":"+ buf;
    str = str +":"+ pCodec->long_name;
    sprintf(buf,"%d", pFormatCtx->duration);
    str = str +":"+ buf;
    sprintf(buf,"%d", pCodecCtx->bit_rate);
    str = str +":"+ buf;
